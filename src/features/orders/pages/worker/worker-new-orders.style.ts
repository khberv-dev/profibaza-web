...
export const WOTitle = styled.h1`
  margin: 0; font-size: 22px; font-weight: 900; color: #12284a; letter-spacing: -0.01em;
`;
export const WOSub = styled.div`
  color: #6b7a90; font-size: 13px; margin-top: 4px;
`;
...
export const WOMid = styled.div`
  grid-area: mid;
  display: grid; gap: 8px; min-width: 0; /* важно для ellipsis */
`;
...
`;
export const WOStatus = styled.span<{ $tone: "blue" | "green" | "amber" | "gray" | "red" }>`
  display: inline-flex; align-items: center; height: 24px; padding: 0 10px;
  border-radius: 999px; font-size: 12px; font-weight: 800;
  ${({ $tone }) => ({
    blue:  { color: "#0b3b8f", background: "#eff6ff", border: "1px solid #dbeafe" },
    green: { color: "#14532d", background: "#ecfdf5", border: "1px solid #a7f3d0" },
    amber: { color: "#92400e", background: "#fff7ed", border: "1px solid #fed7aa" },
    gray:  { color: "#475467", background: "#f2f4f7", border: "1px solid #e7ecf3" },
    red:   { color: "#991b1b", background: "#fef2f2", border: "1px solid #fecaca" },
  }[$tone])};
`;

/* Метаданные */
export const WOMeta = styled.ul`
  display: flex; flex-wrap: wrap; gap: 12px 16px; list-style: none; margin: 2px 0 0; padding: 0;
  li { display: inline-flex; gap: 8px; align-items: center; color: #334155; font-size: 14px; }
  .k { color: #7b8ba5; }
  .v {
    color: #12284a; font-weight: 700;
...
export const WODivider = styled.div`
  height: 1px; background: #eef3fb; margin-top: 4px;
`;
export const WODesc = styled.p`
  margin: 0; color: #0f172a; font-size: 14px; line-height: 1.5;
`;
...
export const WORight = styled.aside`
  ...
  align-content: start; justify-items: stretch;   /* ← кнопки одинаковой ширины */
...
export const WOGhost = styled.button`
  ${btnBase};
  background: #fff; border: 1px solid #e7ecf3; color: #12284a; font-weight: 600;
  &:hover { box-shadow: 0 8px 20px rgba(2,32,71,.08); color: #1e5cfb; border-color: #d0dcfb; }
  &:active { transform: translateY(1px); }
`;
export const WOPrimary = styled.button`
  ${btnBase};
  height: 40px; color: #fff; font-weight: 600; background: #2f6bff; border: 1px solid #2f6bff;
...
export const WODanger = styled.button`
  ${btnBase};
  color: #b91c1c; background: #fff; border: 1px solid #fecaca; font-weight: 800;
  &:hover { box-shadow: 0 8px 18px rgba(185,28,28,.07); }
  &:active { transform: translateY(1px); }
`;

/* Пусто / загрузка */
export const WOEmpty = styled.div`
  display: grid; gap: 8px; place-items: center; padding: 32px 18px;
  background: #fff; border: 1px dashed #d8e1fb; border-radius: 16px; color: #6b7a90;
`;
