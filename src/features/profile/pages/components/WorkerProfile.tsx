...
export const WorkerProfile: React.FC = () => {
  ...
  const profOptions: SelectOption[] = useMemo(
    () =>
      professions.map((p) => ({
        ...
        label: profLabel(p),
        disabled:
          mode === "create" && createdIds.has(p.id) && p.id !== professionId,
      })),
    [professions, mode, createdIds, professionId]
  );

  // только созданные профессии
  const createdList = useMemo(
    () => professions.filter((p) => createdIds.has(p.id)),
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setProfessions(opts);
        if (current) {
          setWorkerProfRaw(current as any);
          if (current?.id) {
            try {
              const items = await getProfessionDemos(current.id, ac.signal);
              setDemosByRowId((prev) => ({ ...prev, [current.id]: items }));
            } catch {
              /* noop */
            }
          }
          setCreatedIds(new Set([current.professionId]));
        }
      } finally {
  ...
  const openEditFor = (profId: string) => {
    if (workerProfRaw && workerProfRaw.professionId === profId) {
      setProfessionId(workerProfRaw.professionId || "");
      setMinPrice(String(workerProfRaw.minPrice ?? ""));
      setMaxPrice(String(workerProfRaw.maxPrice ?? ""));
      setHasTeam(Boolean(workerProfRaw.hasTeam));
      setTeamMemberCount(String(workerProfRaw.teamMemberCount ?? "1"));
      setReadyForHugeProject(Boolean(workerProfRaw.readyForHugeProject));
      const comp = (workerProfRaw as any)?.competitions;
      setCompetitions(comp === "YES" || comp === "NO" ? comp : "NO");
      setInventory(((workerProfRaw as any)?.inventory as string) ?? "");
      getProfessionDemos(workerProfRaw.id)
        .then((items) => {
          setDemosByRowId((prev) => ({ ...prev, [workerProfRaw.id]: items }));
        })
        .catch(() => {});
    } else {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // если редактируем — пробуем PUT /worker/profession/:id
      if (mode === "edit") {
        const rowId =
          workerProfRaw && workerProfRaw.professionId === professionId
            ? workerProfRaw.id
            : undefined;

        if (rowId) {
          await updateWorkerProfession(rowId, payload);
        } else {
          // fallback, если id нет — создаём новую запись
      ...
      // обновим локально
      setCreatedIds((prev) => new Set(prev).add(professionId));
      const cur = await getWorkerProfession().catch(() => null);
      setWorkerProfRaw((cur as any) || null);
      if (cur?.id) {
        try {
          const items = await getProfessionDemos(cur.id);
          setDemosByRowId((prev) => ({ ...prev, [cur.id]: items }));
        } catch {
          /* noop */
        }
      }
          ...
          <Card>
            <CardHeader>
              ...
              <GhostBtn
                ...
                disabled={!hasAvailableForCreate}
                title={
            ...
            <CardBody>
              {createdList.length > 0 ? (
                <div style={{ display: "grid", gap: 12 }}>
                  {createdList.map((p) => {
                    const label = profLabel(p);
                    const f = getFormFor(p.id, workerProfRaw);
                    return (
                      <JobItem key={p.id}>
                        ...
                        <div>
                          ...
                          <StatRow>
                            ...
                            <StatPill>
                              ...
                              <span>
                                {f.hasTeam ? "в команде" : "команды нет"}
                              </span>
                            </StatPill>
                          </StatRow>
                        </div>
                        ...
                        <ActionRow>
                          <PrimaryBtn
                            type="button"
                            onClick={() => openEditFor(p.id)}
                          >
                            Редактировать
                          </PrimaryBtn>
                          <SoftPill>
                            Крупные проекты: {f.huge ? "Да" : "Нет"}
                          </SoftPill>
                          <SoftPill>
                            Конкурсы: {f.competitions ? "Да" : "Нет"}
                          </SoftPill>
                        </ActionRow>
                        {/* 
                        <LinkRow>
                          <TextLink
                            onClick={() => alert("Изменить видимость: TODO")}
                          >
                            Изменить видимость
                          </TextLink>
                          <TextLink onClick={() => alert("Дублировать: TODO")}>
                            Дублировать
                          </TextLink>
                        </LinkRow> */}
                        ...
                        <MutedBar>
                          <b>Инструменты:</b>{" "}
                          {f.inventory ? f.inventory : "не указаны"}
                        </MutedBar>
                      </JobItem>
                    );
                  })}
                ...
                <Notice tone="neutral">
                  Пока ни одной профессии не добавлено. Нажмите «Создать новую»,
                  чтобы добавить.
                </Notice>
              )}
            </CardBody>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <Upload style={{ marginTop: 8 }}>
            ...
            <input
              ...
              // блокируем, если нет rowId (нужно сначала сохранить запись)
              disabled={
                !(workerProfRaw && workerProfRaw.professionId === professionId)
              }
            />
