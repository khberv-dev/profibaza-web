...
// --- над компонентом, рядом с остальными константами ---
...
// список для фильтра — линейный массив из всех групп
const allProfs = PROF_GROUPS.flatMap((g) => g.items);
const filtered = allProfs.filter((x) =>
...
useEffect(() => {
  const onDown = (e: MouseEvent) => {
    ...
    if (!el) return;
    // если клик вне блока с инпутом/тегами/дропдауном — закрываем
  ...
  <HotRow>
    {HOT_PROF.map((p) => (
      <HotChip
        ...
        onClick={() =>
          setProf((s) => (s.includes(p) ? s.filter((x) => x !== p) : [...s, p]))
        }
      >
        {p}
      </HotChip>
              ...
              <div key={g.name}>
                ...
                {items.map((opt) => {
                  ...
                  return (
                    ...
                    key={opt}
                    aria-checked={active}
                    onMouseDown={(e) => e.preventDefault()} // не даём инпуту потерять фокус
                    onClick={() => {
  ...
  <Hint>Можно выбрать несколько направлений. Начните ввод, чтобы отфильтровать.</Hint>
          ...
          <Row>
            <Field>
              ...
              <SwitchWrap>
                <SwitchBtn active={readyBig} onClick={() => setReadyBig(true)}>Да</SwitchBtn>
                <SwitchBtn active={!readyBig} onClick={() => setReadyBig(false)}>Нет</SwitchBtn>
              </SwitchWrap>
            </Field>
            ...
            <Field>
              ...
              <Inline>
                <SwitchWrap>
                  <SwitchBtn active={hasTeam} onClick={() => setHasTeam(true)}>Да</SwitchBtn>
                  <SwitchBtn active={!hasTeam} onClick={() => setHasTeam(false)}>Нет</SwitchBtn>
                </SwitchWrap>
                <Input
          ...
          <Row>
            <Field>
              ...
              <SwitchWrap>
                <SwitchBtn active={hasTools} onClick={() => setHasTools(true)}>Да</SwitchBtn>
                <SwitchBtn active={!hasTools} onClick={() => setHasTools(false)}>Нет</SwitchBtn>
              </SwitchWrap>
              {hasTools && (
                <>
                  <div style={{ height: 8 }} />
          ...
          <Row>
            <Field>
              ...
              <SwitchWrap>
                <SwitchBtn active={contest} onClick={() => setContest(true)}>Да</SwitchBtn>
                <SwitchBtn active={!contest} onClick={() => setContest(false)}>Нет</SwitchBtn>
              </SwitchWrap>
              {contest && (
                <>
                  <div style={{ height: 8 }} />
